...
if __name__ == '__main__':
    ...
    fps = 2 # how many inferences the program will make per second
    max_d = .1 # maximum distance the drone will move with any 1 inference (meters)
    kp = {'frame_id': -1}
    print('Initializing Cube Commander')
    # Initialize the CubeCommander
    demo = FollowCommander()
    demo.flight_manager.fly_to_position(0.0, 0.0, FLIGHT_ALT)
    ...
    if rospy.is_shutdown():
        ...
        exit(1)
    print('Offboard Mode Detected: Waiting 10 seconds')
    print('Retrieving starting distance')
    demo.flight_manager.wait(10)
    conf = 0

    while conf < 0.5:
        size, dyaw_t, conf = get_body_info()
        distance_start = get_distance(size)
        rospy.sleep(1/fps)
    while True:
        conf = 0
        while conf < 0.5:
            size, dyaw_t, conf = get_body_info()
            distance_t = get_distance(size) - distance_start
            rospy.sleep(1/fps)
        ...
        demo.flight_manager.fly_to_position(x,y,FLIGHT_ALT,ryaw)
        rospy.sleep(1/fps)
