...
class SpeedCommander:
    def __init__(self, waypoints, velocities):
        ...
        print('Initializing flight to 0 0 ALT')
    ...
    def fly_to_next_waypoint(self):
        ...
        print(f'Flying to position X:{x} Y:{y} Z;{z} at velocity {velx} {vely} {velz}')
        # Increment the current waypoint index
        self.current_waypoint_index = (self.current_waypoint_index + 1) % (len(self.waypoints)-1)
 
    def go_to_first_position(self):
        # flies to first waypoint with unspecified velocity
        self.flight_manager.fly_to_position(0.5,0.5,FLIGHT_ALT)
        print('self.flight_manager.fly_to_position(0.5,0.5,FLIGHT_ALT)')
    def __del__(self):
